...
export const AdminRealtorManagement: React.FC<AdminRealtorManagementProps> = ({ 
  ...
  const [selectedRealtor, setSelectedRealtor] = useState<AdminRealtorResponse | null>(null);
  ...
  const filteredRealtors = realtorsData?.data?.filter((realtor) => {
      ...
      {analytics && !analyticsLoading && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-sm font-medium text-gray-600">Total Realtors</div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-sm font-medium text-gray-600">Active Realtors</div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="text-2xl font-bold text-green-600 mt-1">
                ₦{(analytics.overview.revenue.total * analytics.overview.revenue.growth / 100).toLocaleString()}
              </div>
            ...
            <div>
              <div className="text-sm font-medium text-gray-600">Commission Earned</div>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="text-gray-500">
                      ...
                      <h3 className="mt-2 text-sm font-medium text-gray-900">No realtors found</h3>
                  ...
                  <tr key={realtor.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-sm text-gray-500">{realtor.user.email}</div>
                          <div className="text-sm text-gray-500">{realtor.user.phone}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {realtor.businessAddress && (
                        <div className="text-sm text-gray-500">{realtor.businessAddress}</div>
                      )}
                      {realtor.corporateRegNumber && (
                        <div className="text-xs text-gray-500">CAC: {realtor.corporateRegNumber}</div>
                      )}
                    </td>
                    
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        ${adminService.getRealtorStatusColor(realtor.status) === 'green' ? 'bg-green-100 text-green-800' :
                          adminService.getRealtorStatusColor(realtor.status) === 'yellow' ? 'bg-yellow-100 text-yellow-800' :
                          adminService.getRealtorStatusColor(realtor.status) === 'red' ? 'bg-red-100 text-red-800' :
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2">{adminService.getRealtorStatusLabel(realtor.status)}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Email Verified:</span>
                <span className="ml-2">{realtor.user.isEmailVerified ? 'Yes' : 'No'}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Joined:</span>
                <span className="ml-2">{new Date(realtor.createdAt).toLocaleDateString()}</span>
              </div>
              <div>
            ...
            <div className="space-y-3 text-sm">
              {realtor.businessName && (
                <div>
                  ...
                  <span className="ml-2 text-gray-900">{realtor.businessName}</span>
                </div>
              )}
              {realtor.businessAddress && (
                <div>
                  <span className="font-medium text-gray-700">Business Address:</span>
                  <span className="ml-2 text-gray-900">{realtor.businessAddress}</span>
                </div>
              )}
              {realtor.corporateRegNumber && (
                <div>
                  <span className="font-medium text-gray-700">CAC Number:</span>
                  <span className="ml-2 text-gray-900">{realtor.corporateRegNumber}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg max-w-lg w-full">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <input
              type="date"
              value={formData.suspendedUntil ? new Date(formData.suspendedUntil).toISOString().split('T')[0] : ''}
              onChange={(e) => setFormData(prev => ({ 
                ...prev, 
                suspendedUntil: e.target.value ? new Date(e.target.value) : undefined 
              }))}
              className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <p className="text-green-800 text-sm">
              You are about to approve this realtor's account. They will be able to list properties 
              and receive bookings after approval.
            </p>
          </div>
